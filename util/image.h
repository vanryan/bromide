#ifndef BROMIDE_IMAGE_H
#define BROMIDE_IMAGE_H
...
Halide::Image<T> transpose(Halide::Image<T> arr) {
    Halide::Image<T> arr2(arr.height(), arr.width(), arr.channels());

    for(int k = 0; k < arr.channels(); k++) {
        for (int j = 0; j < arr.height(); j++) {
          for (int i = 0; i < arr.width(); i++) {
            arr2(j, i, k) = arr(i, j, k);
          }
        }
    }
...
template<class T = float>
Halide::Image<T> blob_to_image_2(const caffe::BlobProto& blob, int num, int channels, int height, int width, int dim = 0) {
...
template<class T = float>
Halide::Image<T> blob_to_image(const caffe::BlobProto& blob, int dim=0) {
